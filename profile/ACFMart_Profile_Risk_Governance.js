...
// Kiểm tra môi trường để xử lý export file phù hợp (Node.js vs Browser)
...
// Margin: Top/Right/Bottom: 2cm (1134 DXA), Left: 3cm (1701 DXA)
...
function bold(text, size=24) { return createRun(text, { bold: true, size }); }
function normal(text, size=24) { return createRun(text, { size }); }
function italic(text, size=24) { return createRun(text, { italics: true, size }); }

// --- Paragraph Helpers ---
function para(children, opts={}) {
  // Nếu children là string, chuyển thành array chứa TextRun
  const content = typeof children === 'string' ? [normal(children)] : children;
...
function paraCenter(text, size=24) {
  return para([normal(text, size)], { alignment: AlignmentType.CENTER });
}
  ...
  return new Table({
    ...
    rows: rows.map(row => new TableRow({
      children: row.map((cell, i) => {
        // Xử lý nội dung cell linh hoạt
        let cellChildren = [];
        if (Array.isArray(cell.content)) {
          cellChildren = cell.content;
        } else if (Array.isArray(cell.runs)) {
          cellChildren = [para(cell.runs)];
        } else if (cell.text !== undefined) {
          cellChildren = [para([normal(cell.text, 22)])];
        } else if (typeof cell === 'string') {
          cellChildren = [para([normal(cell, 22)])];
        }

        return new TableCell({
          borders: cell.borders || BORDERS,
          width: { size: colWidths[i], type: WidthType.DXA },
          margins: cell.margins || cellM,
...
function doc1() {
  const body = [
    ...
    // Đã sửa: Loại bỏ logic .then thừa
    makeTable([
      [{ runs: [bold('STT',22)], fill:'2E75B6' }, { runs: [bold('Tên ứng dụng',22)], fill:'2E75B6' }, { runs: [bold('Địa chỉ truy cập/lưu trữ',22)], fill:'2E75B6' }, { runs: [bold('Logo',22)], fill:'2E75B6' }],
      [{ text: '1' }, { text: 'Sàn TMĐT ACFMart (Môi trường Demo)' }, { text: URL_DEMO }, { text: '[Đính kèm]' }],
      [{ text: '2' }, { text: 'Sàn TMĐT ACFMart (Môi trường Production)' }, { text: URL_PROD }, { text: '[Đính kèm]' }],
    ], [600, 3200, 3426, 1200]),
  ...
  return new Document({
    ...
    sections: [{ ...pageProps(), children: filtered }],
  });
}

// ═══════════════════════════════════════════════════════════════════════════════
// TÀI LIỆU 2: ĐỀ ÁN TMĐT
// ═══════════════════════════════════════════════════════════════════════════════
function doc2() {
  const children = [
    ...hdr(),
...
// ═══════════════════════════════════════════════════════════════════════════════
// TÀI LIỆU 5: CAM KẾT CHỐNG HÀNG GIẢ
// ═══════════════════════════════════════════════════════════════════════════════
function doc5() {
  const children = [
    ...hdr(),
...
// ── GENERATE ALL ──────────────────────────────────────────────────────────────
async function main() {
  // Cấu hình thư mục output - có thể thay đổi tùy môi trường
  const OUT_DIR = isNode ? '/mnt/user-data/outputs' : './outputs';
  
  // Tạo thư mục nếu chưa tồn tại (chỉ trong Node.js)
  if (isNode && !fs.existsSync(OUT_DIR)) {
  ...
  for (const d of docs) {
    try {
      ...
      if (isNode) {
        const filePath = path.join(OUT_DIR, d.name);
        fs.writeFileSync(filePath, buffer);
        console.log(`✅ Generated (Node): ${d.name} at ${filePath}`);
      } else {
        // Xử lý cho môi trường Browser
        ...
        console.log(`✅ Generated (Browser): ${d.name}`);
      }
    } catch (error) {
      console.error(`❌ Error generating ${d.name}:`, error);
    }
  }
  console.log('\n🎉 Done — All files processed.');
}

// Export functions để có thể sử dụng module này ở nơi khác
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { doc1, doc2, doc3, doc4, doc5, main };
}

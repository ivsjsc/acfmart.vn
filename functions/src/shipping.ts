...
type WebhookPayload = {
  ...
  fee?: number | string
  pick_money?: number | string
  return_part_package?: number | string
}
...
function normalizeWebhookBody(req: RawBodyRequest): WebhookPayload {
...
function mapGhtkStatus(statusId: number): { orderStatus?: string; label: string } {
  switch (statusId) {
    case -1:
...
async function updateOrdersByParentCode(
  parentCode: string,
  updater: (
...
export const registerShipment = onCall(
  { region: "asia-southeast1" },
  async (request) => {
    if (!request.auth?.uid) {
      throw new HttpsError("unauthenticated", "Vui lòng đăng nhập để đồng bộ vận đơn")
    }

    const input = request.data as RegisterShipmentInput
    ...
    const providerName = asString(
      input?.providerName,
      providerId === "ghtk" ? "Giao Hàng Tiết Kiệm (GHTK)" : providerId.toUpperCase()
    )
    const serviceCode = asString(input?.serviceCode)

    if (!orderCode || !trackingNumber) {
      throw new HttpsError("invalid-argument", "Thiếu orderCode hoặc trackingNumber")
    }

    const updatedCount = await updateOrdersByParentCode(orderCode, (_ref, data) => {
      if (data.customerId && data.customerId !== request.auth!.uid) {
        throw new HttpsError("permission-denied", "Bạn không có quyền cập nhật đơn này")
      }
    ...
    if (updatedCount === 0) {
      throw new HttpsError("not-found", "Không tìm thấy đơn hàng cần đồng bộ vận đơn")
    }
...
export const ghtkWebhook = onRequest(
  { region: "asia-southeast1" },
  async (req, res) => {
    if (req.method !== "POST") {
    ...
    const payload = normalizeWebhookBody(req as RawBodyRequest)

    ...
    <div className="container-acf py-4 lg:py-6">
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_360px]">
          ...
          <div className="card overflow-hidden">
            <div className="bg-gradient-to-r from-brand-red-500 to-brand-red-600 p-5 text-white">
              <div className="flex flex-wrap items-center gap-3">
                <span className={cn('rounded-md px-2.5 py-0.5 text-xs font-bold', status.color)}>
                  {status.label}
                </span>
              ...
              {order.trackingNumber && (
                <div className="mt-2 flex items-center gap-2 text-sm text-white/90">
                  Mã vận đơn: <strong className="font-mono">{order.trackingNumber}</strong>
                  <button onClick={copyTracking} className="rounded-md p-1 hover:bg-white/10">
                    <Copy size={12} />
                  </button>
                </div>
              )}
                ...
                return (
                  <div key={i} className="relative flex gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      {step.note && (
                        <div className="mt-0.5 text-xs text-neutral-600">{step.note}</div>
                      )}
                    </div>
                  </div>
                );
          ...
          {order.requiresShipping ? (
            <div className="card p-5">
              ...
              <div className="text-sm leading-relaxed">
                <div className="font-semibold text-neutral-900">{order.shippingAddress.name}</div>
            ...
            <div className="card p-5">
              ...
              <div className="rounded-lg bg-sky-50 p-3 text-sm text-sky-700">
                Đơn hàng này không có vận chuyển. Shop sẽ kích hoạt hoặc bàn giao quyền truy cập
                dịch vụ sau khi thanh toán được xác nhận.
              </div>
            </div>
          )}
            ...
            <div className="card p-5">
              ...
              <div className="space-y-3 text-sm">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span
                    className={cn('rounded-full px-2.5 py-1 text-xs font-semibold', status.color)}
                  >
                    {status.label}
                  </span>
                  {refundStatusMeta && (
                ...
                <p className="text-xs leading-6 text-neutral-500">
                  Yêu cầu trả hàng/hoàn tiền được xử lý theo ledger nội bộ của ACFMart. Nếu hoàn
                  tiền qua cổng thanh toán được hỗ trợ, trạng thái sẽ cập nhật ở đây theo webhook
                  backend.
                </p>
              </div>
            </div>
              ...
              {order.items.map((item) => (
                <div key={item.variantId} className="flex gap-3 py-3 first:pt-0 last:pb-0">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-0.5 text-xs text-neutral-500">{item.shopName}</div>
                    <div className="mt-1 flex items-center justify-between">
                      <span className="text-xs text-neutral-600">x{item.quantity}</span>
        ...
        <aside className="space-y-4 lg:sticky lg:top-24 lg:self-start">
          {/* Summary */}
          <div className="card p-5">
            <h2 className="mb-3 text-base font-bold text-neutral-900">Thanh toán</h2>
            <div className="space-y-2 text-sm">
              ...
              <div className="text-xs text-neutral-500">
                Đã thanh toán qua{' '}
                <strong className="text-neutral-700">{order.paymentMethod}</strong>
              </div>
            </div>
          </div>
          ...
          {/* Actions */}
          <div className="card p-5">
            <h2 className="mb-3 text-base font-bold text-neutral-900">Hành động</h2>

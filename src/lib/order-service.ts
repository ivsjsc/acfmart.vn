...
import { type ShippingOriginPayload } from './warehouse-routing';
...
function mapOrderDoc(id: string, data: Record<string, any>): OrderDoc {
  return {
    ...
    shippingReason: typeof data.shippingReason === 'string' ? data.shippingReason : undefined,
    ...
    shippingReturnPartPackage:
      typeof data.shippingReturnPartPackage === 'number'
        ...
        : undefined,
    shippingLabelUrl: typeof data.shippingLabelUrl === 'string' ? data.shippingLabelUrl : undefined,
...
function validateMarketplaceOrderInput(input: CreateMarketplaceOrdersInput): void {
  ...
  const requiresShipping = input.requiresShipping ?? input.items.some(cartItemRequiresShipping);
  if (isCod && input.paymentStatus !== 'cod') {
    throw new Error('Đơn COD phải có trạng thái thanh toán COD');
  }
  if (isCod && !input.items.every(cartItemAllowsCod)) {
    throw new Error('Sản phẩm số/SaaS không hỗ trợ COD');
  }
  if (!isCod && input.paymentStatus !== 'pending') {
    throw new Error('Đơn online phải chờ xác nhận thanh toán từ hệ thống');
  }

  const moneyFields = [input.shippingFee, input.codFee, input.discountTotal ?? 0];
  if (!moneyFields.every(isNonNegativeCurrencyAmount)) {
    throw new Error('Số tiền của đơn hàng không hợp lệ');
  }
  if (!requiresShipping && (input.shippingFee > 0 || input.codFee > 0)) {
    throw new Error('Đơn không vận chuyển không được có phí ship hoặc phí COD');
  }

  if (requiresShipping && !normalizeShippingOrigin(input.shippingOrigin)) {
...
export async function createMarketplaceOrders(
  ...
  const shippingWeights = entries.map(([, items]) =>
    items
      .filter(cartItemRequiresShipping)
      .reduce((sum, item) => sum + item.price * item.quantity, 0)
  );
  const allocatedShipping = allocateAmount(input.shippingFee, shippingWeights);
  ...
  const inputRequiresShipping =
    input.requiresShipping ?? input.items.some(cartItemRequiresShipping);
  const shippingOrigin = inputRequiresShipping
    ? normalizeShippingOrigin(input.shippingOrigin)
    : undefined;
  if (inputRequiresShipping && !shippingOrigin) {
    throw new Error('Thông tin kho hàng không hợp lệ');
  }
  ...
  for (const [index, [shopId, items]] of entries.entries()) {
    ...
    const groupRequiresShipping = items.some(cartItemRequiresShipping);
    const order: Omit<OrderDoc, 'id'> = {
      ...
      shippingMethod: groupRequiresShipping ? input.shippingMethod : NO_SHIPPING_METHOD_NAME,
      requiresShipping: groupRequiresShipping,
      shippingProviderId: groupRequiresShipping ? input.shippingProviderId : undefined,
      shippingProviderName: groupRequiresShipping ? input.shippingProviderName : undefined,
      shippingServiceCode: groupRequiresShipping ? input.shippingServiceCode : undefined,
      shippingOrigin: groupRequiresShipping ? shippingOrigin : undefined,
      ...
      timeline: [
        {
          ...
          note:
            status === 'payment_pending'
              ...
              : groupRequiresShipping
                ? 'Đơn hàng mới chờ seller xác nhận'
                : 'Đơn dịch vụ mới chờ shop kích hoạt',
        },
      ],
      created_at: now,
      updated_at: now,
    ...
    const orderForWrite = stripUndefinedFields(order as Record<string, unknown>) as Omit<
      OrderDoc,
      'id'
    >;
...
export async function listBuyerOrders(params: {
  ...
  const snap = await getDocs(query(ordersCol, where('customerId', '==', params.customerId)));
  return snap.docs
    .map((d) => mapOrderDoc(d.id, d.data()))
    .sort((a, b) => orderCreatedAtMs(b) - orderCreatedAtMs(a))
...
export async function getSellerOrderByCode(code: string, shopId: string): Promise<OrderDoc | null> {
...
export async function getBuyerOrderByCode(
  ...
  const snap = await getDocs(
    query(ordersCol, where('code', '==', code), where('customerId', '==', customerId), limit(1))
  );
  if (snap.empty) return null;
  const order = snap.docs[0];
  return mapOrderDoc(order.id, order.data());
...
export async function createReturnRequest(
  ...
  const refundAmount = selected.reduce((sum, item) => sum + item.price * item.quantity, 0);

...
export async function downloadSellerRegistrationDocument(data: SellerDocumentData) {
  ...
  const children: DocChild[] = [
    ...
    bullet(docx, "Đồng ý để ACFMart/IVS JSC sử dụng thông tin hồ sơ cho mục đích xác minh và vận hành gian hàng."),
...
export async function downloadSellerContractDocument(data: SellerDocumentData) {
  ...
  const children: DocChild[] = [
    ...
    bullet(docx, "ACFMart/IVS JSC được quyền tạm khóa gian hàng, gỡ sản phẩm hoặc yêu cầu bổ sung hồ sơ khi phát hiện rủi ro tuân thủ."),

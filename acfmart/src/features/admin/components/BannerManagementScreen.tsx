...
export function BannerManagementScreen() {
  ...
  const [newBanner, setNewBanner] = useState({
    ...
    position: 0,
    active: true,
    source: "firestore" as const, // Thêm trường source
  })
  ...
  async function handleAddBanner() {
    ...
    try {
      await createBanner({
        ...
        source: newBanner.source, // Đảm bảo truyền source
      })
      toast.success("Đã thêm banner")
      setNewBanner({ image_url: "", link_url: "", title: "", position: 0, active: true, source: "firestore" })
      await load()
    } catch {
    ...
    <div className="container-acf py-8">
      ...
      <section className="mb-10 rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          <button
            type="button"
            onClick={() => setNewBanner({ image_url: "", link_url: "", title: "", position: 0, active: true, source: "firestore" })}
